...
**RG-Aware:** A pre-conformance adoption tier defined by the standard for organizations that collect linked content from same-tenant storage when identified during preservation workflows, generate structured exceptions for uncollectable content, and disclose preservation and version-resolution limitations through ESI protocols or equivalent mechanisms. RG-Aware does not constitute Reconstruction-Grade conformance but establishes a transparent, structured starting point on the maturity path toward RG-Core. See Section 6.2 and Appendix B, Section B.0.
